...
for vals in history.values():
    vals.sort()
today=[appt for appt in all_appts if appt.get('calendar','').strip().lower()=='ernie' and datetime.datetime.fromisoformat(appt['datetime']).astimezone(zone).date()==datetime.date(2026,2,9)]
today.sort(key=lambda appt: appt['datetime'])
print(f"Ernie Monday update {target}:")
if not today:
    print('No Ernie lessons yet')
else:
    for appt in today:
