...
all_appts=[]
while True:
    params={'minDate':start_date,'maxDate':target_date,'limit':limit,'offset':offset,'cancelled':'false'}
...
for vals in history.values():
    vals.sort()
today=[appt for appt in all_appts if appt.get('calendar','').strip().lower()=='ernie' and datetime.datetime.fromisoformat(appt['datetime']).astimezone(zone).date()==datetime.date(int(target_date[:4]),int(target_date[5:7]),int(target_date[8:10]))]
today.sort(key=lambda appt: appt['datetime'])
print(f"Ernie Friday update {target_date}:")
if not today:
    print('No Ernie lessons on that day yet.')
else:
    for appt in today:
